...
class LevelMapperwithArea(object):
    ...
    def __call__(self, boxlists):
        ...
        target_lvls = torch.ceil(self.k_max - torch.log2(img_area / bbox_area + self.eps))
        target_lvls = torch.clamp(target_lvls, min=self.k_min, max=self.k_max)
        return target_lvls.to(torch.int64) - self.k_min
...
class Pooler(nn.Module):
    ...
    def __init__(self, output_size, scales, sampling_ratio, lvl_map_func):
        """
        Arguments:
    ...
    def forward(self, x, boxes):
        ...
        # levels = torch.zeros(boxes[0].bbox.shape[0])
        num_rois = len(rois)
