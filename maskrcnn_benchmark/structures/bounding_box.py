...
class BoxList(object):
    ...
    def max_length(self):
        ...
        return torch.max(w, h).squeeze(1)

    def max_image_size(self):
        return torch.max(torch.as_tensor([self.size[0]], dtype=torch.float, device=self.bbox.device), torch.as_tensor([self.size[1]], dtype=torch.float, device=self.bbox.device))

    def prepare_image_size(self):
        num_box = self.bbox.shape[0]
        max_img_size = self.max_image_size()
        tmp = torch.zeros((num_box, 1), dtype=torch.float, device=self.bbox.device)
        return (max_img_size + tmp).squeeze(1)

    def image_area(self):
        num_box = self.bbox.shape[0]
        input_area = torch.as_tensor(self.size[0] * self.size[1], dtype=torch.float, device=self.bbox.device)
        tmp = torch.zeros((num_box, 1), dtype=torch.float, device=self.bbox.device)
...
if __name__ == "__main__":
    ...
    print(bbox.prepare_image_size())

    print(bbox.image_area())